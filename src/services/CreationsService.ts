...
export class CreationsService {
  constructor(private readonly creationsCollection: Collection<NoId<Creation>>) {}
  ...
  async getFeaturedCreations(): Promise<Creation[]> {
    ...
    const results = await this.creationsCollection
      .find({
  ...
  async createCreation(
    creation: Omit<Creation, 'id' | 'createdAt' | 'updatedAt'>,
  ): Promise<Creation> {
  ...
  async updateCreation(
    id: string,
    update: Partial<Omit<Creation, 'id' | 'createdAt' | 'updatedAt'>>,
  ): Promise<Creation> {
    const updateDoc = {
      $set: {
        ...update,
