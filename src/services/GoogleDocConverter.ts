...
export class GoogleDocConverter {
    ...
    private static convertTags(text: string): string {
        ...
        text = text.replace(/\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2},?\s+\d{4}\b/gi, (match) => {
            return `[[${match}]]`;
        });
    ...
    private static applyParagraphStyle(content: string, style?: docs_v1.Schema$ParagraphStyle): string {
        if (!style) {
            return content;
        }
