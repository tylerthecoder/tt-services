...
export class GooglePushService {
  ...
  async pushNoteToGoogleDrive(
    ...
    try {
      ...
      if (isGoogleNote) {
        return await this.addToExistingGoogleDoc(
          note as GoogleNote,
          googleDocRequest,
          userId,
          options.tabName,
        ...
        return await this.createNewGoogleDoc(
          note,
          googleDocRequest,
          userId,
          options.convertToGoogleNote,
        );
      }
    } catch (error) {
  ...
  async pushMultipleNotesToGoogleDoc(
    ...
    try {
      this.logger.info(
        { noteIds, userId, documentTitle },
        'Pushing multiple notes to Google Drive',
      );
